...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def _job_rename(self):
        """Rename a document.

        Nothing on disk moves. The output path follows the source file's name,
        so the title is only what the lists, the job bar and an exported
        folder call this document -- which is exactly what makes renaming safe
        to do from a phone.
        """
        req = self._read_json()
        jid = req.get("id")
